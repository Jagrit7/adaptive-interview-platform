...
17. **The assistant's file-writing tool writes to its own sandbox, never the user's actual machine, even when given a real-looking absolute Windows path.** Every file meant for the real project has had to be downloaded and manually placed. This has caused real mistakes (see #11 in the original list) and needs to stay top of mind every time a file is generated for this project.
